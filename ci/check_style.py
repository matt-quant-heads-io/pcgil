...
def _glob_files(directories, extensions):
    ...
    Args:
        directories: list of directories, relative to the root repo directory.
...
class PythonFormatter:
    ...
    def _apply_style(file_path, style_config):
        _, _, _ = yapf.yapflib.yapf_api.FormatFile(
            file_path, style_config=style_config, in_place=True
        )
...
class JupyterFormatter:
    ...
    def run(self, do_apply_style, no_parallel, verbose):
        ...
        if no_parallel:
            is_valid_files = map(
                partial(
                    self._check_or_apply_style,
                    style_config=self.style_config,
                    do_apply_style=False,
                ),
                self.file_paths,
            )
        else:
        ...
        for is_valid, file_path in zip(is_valid_files, self.file_paths):
            if not is_valid:
                ...
                if do_apply_style:
                    self._check_or_apply_style(
                        file_path, style_config=self.style_config, do_apply_style=True
                    )
        print("Formatting takes {:.2f}s".format(time.time() - start_time))
...
if __name__ == "__main__":
    ...
    python_formatter = PythonFormatter(
        _glob_files(PYTHON_FORMAT_DIRS, ["py"]), style_config=python_style_config
    )
    jupyter_formatter = JupyterFormatter(
        _glob_files(JUPYTER_FORMAT_DIRS, ["ipynb"]), style_config=python_style_config
    )

    changed_files = []
    changed_files.extend(
        python_formatter.run(
